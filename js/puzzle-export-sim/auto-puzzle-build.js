/**
 * Scripted gamemaker-parity puzzle construction: descend-score placement, optional toroidal
 ...
 *
 * Words are stamped in toolbar list order (gamemaker default: descending score). Export uses
...
} from "../gamemaker/pool-order.js";
import { buildGamemakerDictExportPayload } from "../gamemaker/build-export-payload.js";
import {
 ...
 *   diagnoseBuild?: boolean; when true — **`[build-diag]`** JSON lines on stderr (**first failure slot + phase**, export/verify) without full `--trace-build`
  ...
  for (let attempt = 0; attempt < wholeBuildAttempts; attempt++) {
      ...
      if (step > 0 && shiftBetween && interWordShiftMode === "exhaustive16") {
        ...
        if (!exPick) {
          ...
          line("placement_fail", {
            attempt,
            slot,
            phase: "exhaustive16_no_placement",
            word: w,
          });
          failed = true;
          break;
        ...
        for (let ci = 0; ci < seqCandidates.length; ci++) {
          ...
          const pref = scorePlacementCoverExisting(shiftedBoard, probeCur.pathFlat, n);
          let score =
            pref.score +
      ...
      for (let variantIx = 0; variantIx < shiftVariants.length; variantIx++) {
        ...
        const attemptsPerPlacementSample =
          placementCandidateSamples <= 1
            ? maxAttemptsPerWord
            : Math.max(1, Math.floor(maxAttemptsPerWord / placementCandidateSamples));
        let hadCand = false;
        ...
        inner: for (let k = 0; k < innerKMax; k++) {
          ...
          for (let sub = 0; sub < placementCandidateSamples; sub++) {
            ...
            if (!candTry) continue;
            const pref = scorePlacementCoverExisting(gameBoard, candTry.pathFlat, n);
          ...
          if (lookaheadPlacementGate && step + 1 < PERFECT_HUNT_WORD_COUNT) {
            ...
            const laMax =
              typeof opts.lookaheadAttempts === "number" && opts.lookaheadAttempts > 0
    ...
    if (opts.skipForwardVerify !== true) {
      const vrf = verifyForwardPuzzleIfCoveredChain(
        ...
        shiftBetween ? { fillEmptyPathCells: true } : undefined
      );
      if (!vrf.ok) {
        tally.fail_verify++;
